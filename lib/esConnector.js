 ...
 * @returns {object} 
 */
ESConnector.prototype.deleteMapping = function (model) {
    var self = this;
    self.elasticServer.indices.deleteMapping({
...
ESConnector.prototype.destroyAll = function destroyAll(model, filter, done) {
    ...
    if (!filter) {
        throw new Error('Document query not setted!')
    }
...
/**
 * Return number of rows by the where criteria --DONE 
 * @param {String} model name
 * @param {String} filter criteria
 * @param {Function} done callback
 */
...
/**
 * Create a new model instance --DONE
 * @param {String} model name
...
ESConnector.prototype.save = function (model, data, done) {
    ...
    var req = {};
    req['doc'] = self.matchDataToModel(model, data);
...
/**
 * Update a model instance or create a new model instance if it doesn't exist --DONE
 */
...
// TODO: fail, test and re test
    ...
    self.elasticServer.exists(
        ...
        function (response) {
        	if(response == true){
        		self.save(model, data, done);
        	}else{	
        		self.create(model, data, done);
        	}
        }
    );
};
